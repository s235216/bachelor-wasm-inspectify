...
pub struct ReferenceExecution {
    meta: Option<<BiGCLEnv as Env>::Meta>,
    output: Option<Output>,
    annotation: Option<<BiGCLEnv as Env>::Annotation>,
    error: Option<String>,
}
...
pub async fn bigcl_wasm_reference(input_json: String) -> Option<String> {
    ...
    let res = match input_result {
        Ok(input) => {
            let meta = Some(BiGCLEnv::meta(&input));
            let output = BiGCLEnv::run(&input);
            let error = output.as_ref().err().map(|e| e.to_string());
            let output = output.ok();
            let annotation = output
                .as_ref()
                .and_then(|o| BiGCLEnv::validate(&input, o).ok())
                .map(|(_, ann)| ann);
