...
pub struct ReferenceExecution {
    meta: Option<<SignEnv as Env>::Meta>,
    output: Option<Output>,
    annotation: Option<<SignEnv as Env>::Annotation>,
    error: Option<String>,
}

#[wasm_bindgen]
pub async fn sign_wasm_reference(input_json: String) -> Option<String> {
    let input_result: Result<Input, serde_json::Error> = serde_json::from_str(&input_json);
    let res = match input_result {
        Ok(input) => {
            let meta = Some(SignEnv::meta(&input));
            let output = SignEnv::run(&input);
            let error = output.as_ref().err().map(|e| e.to_string());
            let output = output.ok();
            let annotation = output
                .as_ref()
                .and_then(|o| SignEnv::validate(&input, o).ok())
                .map(|(_, ann)| ann);
...
#[wasm_bindgen]
pub async fn sign_wasm_generate(seed: Option<u64>) -> Option<String> {
